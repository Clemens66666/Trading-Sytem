...
def load_data(
    path: str,
    trend_model: dict | None = None,
    entry_model: dict | None = None,
    nrows: int | None = None,
) -> pd.DataFrame:
    raw = next(read_raw_csv(Path(path)))
    if nrows:
        raw = raw.head(nrows)
    bars5 = make_bars(raw, freq="5T")
    df = FeatureBuilder(bars5).build()

    if trend_model is not None:
        bars1h = make_bars(raw, freq="1H")
        feat_h = FeatureBuilder(bars1h).build().drop(columns=["timestamp"])
        p_trend = stack_predict(trend_model, feat_h)
        ser = pd.Series(p_trend, index=bars1h["timestamp"], name="trend_prob_long")
        df["trend_prob_long"] = ser.reindex(df["timestamp"], method="ffill").values

    if entry_model is not None:
        bars10 = make_bars(raw, freq="10T")
        feat_e = FeatureBuilder(bars10).build()
        if trend_model is not None:
            trend_ser = pd.Series(p_trend, index=bars1h["timestamp"], name="trend_prob_long")
            feat_e["trend_prob_long"] = trend_ser.reindex(bars10["timestamp"], method="ffill").values
        feat_e = feat_e.drop(columns=["timestamp"])
        p_entry = stack_predict(entry_model, feat_e)
        ser = pd.Series(p_entry, index=bars10["timestamp"], name="entry_prob_long")
        df["entry_prob_long"] = ser.reindex(df["timestamp"], method="ffill").values
        df["entry_prob_short"] = 1.0 - df["entry_prob_long"]
...
def main(config_path: str = "config.yaml", nrows: int | None = None) -> None:
    ...
    if Path(config["output_paths"]["entry"]).exists():
        with open(config["output_paths"]["entry"], "rb") as f:
            entry_obj = pickle.load(f)["model"]

    df = load_data(
        config["raw_tick_path"],
        trend_model=trend_obj,
        entry_model=entry_obj,
        nrows=nrows,
    )
